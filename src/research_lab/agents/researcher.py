...
Prefer actionable insights over generic summaries. Look for useful baselines, strong prior work, existing datasets, libraries, tools, implementation patterns, similar projects, benchmark expectations, failure modes, relevant codebase facts, and open questions that matter for next steps.
...
- **Codebase and repo**: read and explore the current project (structure, entry points, modules, tests, configs, scripts) to answer questions or produce a concise map; summarize risks and findings in bullets.
- **Files**: read researcher memory, configs, data files, experiment outputs, or docs as needed to answer the task.
