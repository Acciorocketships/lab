...
- **Tier A** (`.airesearcher/state/`): `immediate_plan.md` reflects completed steps; `status.md` and `roadmap.md` match
  outcomes; new long-form content under `.airesearcher/memory/extended/` is linked from Tier A; `lessons.md` records abandonments or
  major learnings when relevant.
- **Branch memory** (`.airesearcher/memory/branch/`): file for the current branch exists and is accurate, or is
...
- **Readmes**: project or researcher readmes (e.g. `.airesearcher/memory/episodes/README.md`) are not left contradictory if behavior
  or layout changed.
