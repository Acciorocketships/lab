...
MEMORY_AND_TIER_A = """**Tier A** means `.airesearcher/data/runtime/state/<file>.md` (not a project-root `state/` folder). Keep those files current, along with `.airesearcher/data/runtime/memory/extended/`.
...
**Tier A files** (under `.airesearcher/data/runtime/state/` — meanings):
...
- **`extended_memory_index.md`** — index of `.airesearcher/data/runtime/memory/extended/` only. Keep this current alongside the rest of Tier A. When you add or materially update a file under that directory, add or refresh the matching index entry. Each entry should point to one long-form file and give a brief description or a few high-level bullets about what the full file contains, so future workers can decide whether to open it.
...
- **`skills_index.md`** — table of `.airesearcher/data/runtime/memory/skills/*.md` entries. Keep aligned when you add, rename, remove, or substantially revise skills.
...
**Branch memory** (`.airesearcher/data/runtime/memory/branch/<b>.md`): branch name `/`→`__`. Per active branch you touch: diverged-from (SHA/tag), purpose, status/results. Merge to **main** → delete file (optional copy to `.airesearcher/data/runtime/memory/extended/`). Abandoned/failed → delete file + note in **`lessons.md`**.

**Episodes** (`.airesearcher/data/runtime/memory/episodes/`): run-by-run packets and worker outputs are auto-populated here. Do not write to this; it is only for looking up context on past subagents actions, if needed.
