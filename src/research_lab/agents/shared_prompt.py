...
**Tier A files** (under `.airesearcher/state/` — meanings):
...
- **`skills_index.md`** — table of `.airesearcher/memory/skills/*.md` entries. Keep aligned when you add, rename, remove, or substantially revise skills.
...
**Branch memory** (`.airesearcher/memory/branch/<b>.md`): branch name `/`→`__`. Per active branch you touch: diverged-from (SHA/tag), purpose, status/results. Merge to **main** → delete file (optional copy to `.airesearcher/memory/extended/`). Abandoned/failed → delete file + note in **`lessons.md`**.

**Episodes** (`.airesearcher/memory/episodes/`): run-by-run packets and worker outputs are auto-populated here. Do not write to this; it is only for looking up context on past subagents actions, if needed.
