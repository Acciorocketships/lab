...
- **`system.md`** — **System-owned only.** Canonical workspace paths (**## Paths**) and **## Recent activity** (recent graph-worker `kind = worker` rows from SQLite `run_events`, oldest first in that list; orchestrator and `/agent` runs omitted). The lab runtime overwrites this file; **do not edit**.
